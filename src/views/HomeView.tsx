...
export function HomeView({ setView }: { setView: (v: string) => void }) {
  return (
    <div className="relative min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-8 overflow-hidden bg-black">
      ...
      <div className="text-center mb-16 relative z-10">
        <h1 className="text-7xl md:text-[10rem] font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-[#ffffff] to-[#a0aab8] select-none pb-2" style={{ textShadow: '0 0 60px rgba(255,255,255,0.15)' }}>
          DPP Forge
        </h1>
        <p className="text-[#8b8fa3] text-lg md:text-xl font-medium tracking-wide mt-2">
          TDS to JSON · Digital Product Passport · QR Generator
        </p>
      </div>

      <div className="flex flex-col md:flex-row gap-6 relative z-10 w-full max-w-4xl">
        <button 
          onClick={() => setView('manual')}
          className="flex-1 group bg-[#1a1d27]/40 backdrop-blur-md border border-[#2e3245] hover:border-white/40 rounded-[2rem] p-8 text-left transition-all duration-500 hover:bg-[#1a1d27]/80"
        >
